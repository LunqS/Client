    ...
    QWidget(parent)
{
    ...
    int activityPageClicked = 0;
    connect(activityBtn, &bigIconButton::clicked, [=]()mutable->void{
        if(activityPageClicked==0){
            activitypage = new activityPage(displayWidget);
            classTable->hide();
            infoWidget->hide();
            clock->hide();
            activitypage->show();
            activitypage->setFixedSize(displayWidget->width(), displayWidget->height());
        }
        activityPageClicked=1;
    });
